...
namespace TI2
{
	class Decoder
	{
		...
		public Decoder(int p, int q)
		{
			PublicKey = CalculatePublicKey((p - 1) * (q - 1));
        ...
        {
			return ExtendedEuclideanAlgorithm(eulerFunction, PublicKey);
        }
		...
		public string DeCodeMessage(string message)
        {
            ...
            {
				result.Append((char)DeCodeSymble(symbol, privateKey, Mod));
            }
